...
We can compare it to the output of mimumum method:
...
Where predictions for rat genes are not available we can obtain predictions for mouse genes and translate them into rat genes through homology. The operation happens automatically if we choose species as 'rno' (Rattus norvegicus)
